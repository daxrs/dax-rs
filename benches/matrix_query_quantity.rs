...
// Same PowerBI matrix-visual shape as matrix_query_currency, but aggregating
// FactSales[Quantity] directly instead of the [Net Sales] measure — there is
// no pre-declared "Total Quantity" measure in the model, so this sums the
// plain column inline. Isolates matrix-pipeline cost (ROLLUPADDISSUBTOTAL x2,
// TOPN, SUBSTITUTEWITHINDEX, NATURALLEFTOUTERJOIN) from the currency-measure's
// own per-group SUMX/SUMMARIZE/LOOKUPVALUE cost.
...
fn matrix_query_quantity_benchmark(c: &mut Criterion) {
    Lazy::force(&ENGINE); // pay parquet-loading cost once, outside the timed loop
    c.bench_function("powerbi_matrix_category_country_quantity", |b| {
...
criterion_group! {
    ...
    targets = matrix_query_quantity_benchmark
}
